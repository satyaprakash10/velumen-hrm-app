<template>
  <teleport to="body">
    ...
    >
      <div
        v-if="visible"
        class="fixed inset-0 z-50 flex bg-black/45 p-3 transition-opacity duration-300 ease-out sm:p-5 md:p-8"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        >
          ...
          >
            <header
              class="flex shrink-0 items-start justify-between gap-4 border-b border-subtle-02 px-6 py-5 sm:items-center sm:px-8 sm:py-6"
            >
              <div class="flex min-w-0 flex-1 flex-col gap-4 sm:flex-row sm:items-center sm:gap-5">
                <p class="shrink-0 text-xs font-normal leading-none text-[#6B7280]">{{ employerName }}</p>
                <span
                ...
                <div class="min-w-0">
                  ...
                  <p class="mt-1 text-sm font-normal leading-snug text-[#6B7280]">TF2019 - Ted Ferguson</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <div
              class="flex min-h-0 flex-1 flex-col px-4 py-5 sm:px-6 sm:py-6 md:px-8 md:py-8"
            >
              <div
              ...
              >
                <div
                  ...
                  class="flex h-[220px] w-full max-w-[720px] flex-1 items-end gap-1.5 sm:gap-2"
                  role="img"
                ...
                >
                  <div
                    ...
                    :key="i"
                    class="flex min-h-0 min-w-0 flex-1 flex-col items-center justify-end gap-2"
                  >
                    <span class="text-[10px] font-medium tabular-nums text-navy/80 sm:text-xs">
                      {{ formatMoneyShort(b.net) }}
                    </span>
                    <div
                      class="w-full max-w-[48px] rounded-t-md bg-gradient-to-t from-[#001738] to-[#1e3a61] transition-all"
                      :style="{ height: `${8 + Math.round((b.pct / 100) * 140)}px` }"
                    />
                ...
                <div
                  v-else
                  class="salary-evolution-art flex w-full max-w-[720px] flex-1 justify-center"
                  aria-hidden="true"
                  v-html="artIllustration"
                />
                <p
                  class="mt-6 max-w-[720px] text-center text-sm font-normal leading-relaxed text-[#6B7280]"
                >
                  {{
                    chartBars.length
...
watch(
  ...
  async v => {
    ...
    if (v) {
      await nextTick()
      const closeBtn = panelRef.value?.querySelector('button[aria-label="Close"]')
      closeBtn?.focus()
    }
  }
)
